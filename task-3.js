...
const styles = [
  // { name: 'font-family', value: '"Paytone One", Helvetica, sans-serif' },
...
function writeStyledText(styles, text) {
  ...
  for (let i = 0; i < styles.length; i++) {
    console.log('styles[i].name', styles[i].name);
...
// // $input.addEventListener('change', writeStyledText(styles, $input.value))
// $input.addEventListener('change', event => {
//   writeStyledText(styles, event.target.value)
// })